...
function makeRoom(p1Name, p1Emoji) {
  return {
    p1: { name: p1Name, emoji: p1Emoji, ready: false },
...
async function joinRoom(code) {
  ...
  try {
    ...
    const data = snap.val();
    if (data.p2 && data.p2.joined) { showJoinError('Room is full. Start a new game.'); return; }
    myRole   = 'p2';
    roomCode = code;
    saveMyIdentity();
    setCodeInURL(code);
...
function onRoomUpdate(data) {
  ...
  if (activeScreen === 'waiting') {
    ...
    if (data.p1 && data.p1.ready && data.p2 && data.p2.ready) {
      // Ensure game phase is set
      if (game.phase === 'waiting_p2' || game.phase === 'setup') {
        if (myRole === 'p1') {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── Auto-rejoin if identity saved ── */
  loadMyIdentity();
  if (roomCode && myRole && FIREBASE_READY) {
    $('share-code').textContent = roomCode;
    const urlCodeNow = getCodeFromURL();
    if (!urlCodeNow) setCodeInURL(roomCode);
    attachRoomListener();
    goTo('waiting');
  }
});
